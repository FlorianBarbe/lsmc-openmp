"""
Analyse complète du modèle LSMC (Séquentiel, OpenMP et Scalabilité)
-------------------------------------------------------------------
Ce script :
  1. Exécute lsmc.exe pour plusieurs tailles N_paths
  2. Compare Séquentiel vs OpenMP (prix + temps)
  3. Calcule le speedup par N_paths
  4. Teste la scalabilité en faisant varier le nombre de threads
  5. Trace 4 graphiques de performance
"""
...
# Paramètres
# -------------------------
N_values = [100 * (i + 1) for i in range(10)]   # 1000 → 20000
...
# -------------------------
# Exécutions séquentiel / OpenMP
# -------------------------
print("=== Convergence Séquentiel vs OpenMP ===")
...
plt.title("Gain de performance par N_paths")
plt.xlabel("Nombre de trajectoires (N)")
...
# 4. Scalabilité OpenMP
plt.subplot(4, 1, 4)
plt.plot(threads_scaling, speedup_scaling, "o-m", label="Speedup mesuré")
plt.plot(threads_scaling, threads_scaling, "--k", label="Speedup idéal (linéaire)")
plt.title(f"Scalabilité OpenMP ({N_paths_scaling} trajectoires)")
