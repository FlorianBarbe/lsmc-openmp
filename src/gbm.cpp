...
void GBM::simulatePaths(double *paths, double S0, double r, double sigma,
...
void GBM::exportCSV(const std::vector<std::vector<double>> &paths,
  ...
  for (size_t p = 0; p < paths.size(); ++p) {
    ...
    if (paths[p].size() != cols) {
      std::cerr << "[AVERTISSEMENT] Ligne " << p
  ...
  try {
    ...
    // Si le rename échoue (Windows si fichier verrouillé), on supprime l'ancien
    // puis on réessaie
    ...
    std::filesystem::remove(filename,
                            ec); // supprime l'ancien sans arrêter le programme
    std::filesystem::rename(tmpname, filename);
  }
}