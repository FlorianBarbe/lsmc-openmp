...
static double getd(const unordered_map<string, string> &a, const string &key,
...
static int geti(const unordered_map<string, string> &a, const string &key,
...
int main(int argc, char **argv) {
  ...
  const int seed = geti(args, "seed", 1234);
  ...
  f << "Threads,S0,K,r,sigma,T,N_steps,N_paths,Prix,Temps,Speedup,RelError,"
  ...
  if (bench == 0) {
    ...
    double price =
        ...
        LSMC::priceAmericanPut(S0, K, r, sigma, T, N_steps, N_paths, seed);
    ...
    f << 1 << "," << S0 << "," << K << "," << r << "," << sigma << "," << T
      ...
      << "," << N_steps << "," << N_paths << "," << price << "," << dt << ","

      << 1.0 << "," << 0.0 << "," << 1.0 << "\n";

    if (g_stop.load())

      cout << "[INFO] STOP reçu -> sortie propre.\n";
  ...
  if (omp_get_max_threads() >= 2)
    ...
    thread_list.push_back(2);

  if (omp_get_max_threads() >= 4)

    thread_list.push_back(4);

  if (omp_get_max_threads() >= 8)

    thread_list.push_back(8);

  if (omp_get_max_threads() >= 16)

    thread_list.push_back(16);
  ...
  for (int Ns : steps_list) {
    ...
    for (int Np : paths_list) {
      ...
      price_seq = LSMC::priceAmericanPut(S0, K, r, sigma, T, Ns, Np, seed);
      ...
      for (int th : thread_list) {
        ...
        double price = LSMC::priceAmericanPut(S0, K, r, sigma, T, Ns, Np, seed);
        ...
        cout << "[OMP] " << th << " threads"
             ...
             << " | Temps = " << dt << " | Speedup = " << speedup

             << " | Prix = " << price << " | Err_rel = " << err_rel
